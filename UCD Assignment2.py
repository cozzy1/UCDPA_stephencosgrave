...
print(result)
result.to_csv(r'C:\Users\DRPhones41\.jupyter\result_JOIN.csv')
...
result.plot(kind='line', x='Passing Yds', y='Year')
plt.xlabel('Year')
...
ax = sns.countplot(y="Game Number", data=result)


# In[ ]:


resgraph = result.groupby(["Player_Name", "Year"])["Passing Yds"].sum()
resgraph.head()
...
resgraph.columns = ["Player_Name", "Year", "Passing Yds"]
resgraph.head()
...
graph = result[(result.Player_Name == "Tom Brady") | (result.Player_Name == "Peyton Manning")]
print.graph()
...
sns.barplot(x = "Passing Yds", y = "Year", data = graph,)
plt.show()
...
result.to_csv(r"C:\Users\DRPhones41\.jupyter\graph1.csv")
