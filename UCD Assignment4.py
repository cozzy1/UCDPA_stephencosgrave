...
# y-axis values
y1 = result["Off Snap Num"]
 
# secondary y-axis values
y2 = result["Fumble Fmb"]
 
# plotting figures by creating axes object
# attempting to use subplots() function
fig, ax = plt.subplots(figsize = (10, 5))
plt.title('Fumbles per snap')
...
ax.set_ylabel('Snaps', color = 'g')
 
# secondary y-axis label
ax2.set_ylabel('Fumble', color = 'g')
...
# n and d are just variable names. Where the formula is dividing by zero, return zero. This stops errors from occurring

def getperstats(n, d):
    return n / d if d else 0

# For each player in our players_list (from the template at the very top). Goes through one by one
# Create all the variables you want to create for year and career. Year ones will help go through the result DF
...
#sumpassingyds = result.groupby(['Year','Player_Name'])['Passing Yds'].sum()
#sumrushingydsd = result.groupby(['Player_Name'])['Rushing Yds'].sum()
